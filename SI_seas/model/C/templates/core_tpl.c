...
void build_psr(struct s_calc *p)
{
    ...
    p->prob = init2d_var_set0(3, tab);
    p->inc = init3u_varp2_set0(3, N_CAC, tab);
...
void step_psr(struct s_X *p_X, double t, struct s_par *p_par, struct s_data *p_data, struct s_calc *p_calc)
{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0;ac<N_AC;ac++) {
            ...
            _sf[1] = correct_rate(par[ORDER_mu_d][routers[ORDER_mu_d]->map[cac]],dt);

            
            _r[0] = _sf[1];
            _r[1] = X[ORDER_I*N_CAC+cac]*par[ORDER_mu_d][routers[ORDER_mu_d]->map[cac]]*par[ORDER_r0][routers[ORDER_r0]->map[cac]]*white_noise__0*(par[ORDER_e][routers[ORDER_e]->map[cac]]*_sf[0]+1)/(X[ORDER_I*N_CAC+cac]+X[ORDER_S*N_CAC+cac]+1);

            sum = _r[1]*dt;
if(sum>0.0){
one_minus_exp_sum = (1.0-exp(-sum));
p_calc->prob[ORDER_S][0] = one_minus_exp_sum*((_r[1]*dt)/sum);
p_calc->prob[ORDER_S][1] = 1.0 - p_calc->prob[ORDER_S][0];
}
else{
p_calc->prob[ORDER_S][0] = 0.0;
p_calc->prob[ORDER_S][1] = 1.0;
}
...
p_calc->prob[ORDER_I][1] = 1.0 - p_calc->prob[ORDER_I][0];
}
else{
p_calc->prob[ORDER_I][0] = 0.0;
p_calc->prob[ORDER_I][1] = 1.0;
}
            ...
            plom_ran_multinomial(p_calc->randgsl, 2, (unsigned int) X[ORDER_S*N_CAC+cac], p_calc->prob[ORDER_S], p_calc->inc[ORDER_S][cac]);
            plom_ran_multinomial(p_calc->randgsl, 2, (unsigned int) X[ORDER_I*N_CAC+cac], p_calc->prob[ORDER_I], p_calc->inc[ORDER_I][cac]);
            ...
            X[ORDER_S*N_CAC+cac] = p_calc->inc[ORDER_S][cac][1];
X[ORDER_I*N_CAC+cac] = p_calc->inc[ORDER_I][cac][1] + p_calc->inc[ORDER_S][cac][0];
...

{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0; ac<N_AC; ac++) {
            ...
            f[0*N_CAC+cac] = X[0*N_CAC+cac] +  ( - (_r[cac][0]*X[ORDER_S*N_CAC+cac]))*dt + - sqrt((_r[cac][0]*X[ORDER_S*N_CAC+cac]))*dem_sto__0[cac];
            f[1*N_CAC+cac] = X[1*N_CAC+cac] +  ( - (_r[cac][1]*X[ORDER_I*N_CAC+cac]) + (_r[cac][0]*X[ORDER_S*N_CAC+cac]))*dt + - sqrt((_r[cac][1]*X[ORDER_I*N_CAC+cac]))*dem_sto__1[cac]+ sqrt((_r[cac][0]*X[ORDER_S*N_CAC+cac]))*dem_sto__0[cac];
        }
    }
    ...
    for (ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for (n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc += ( + (_r[cac][1]*X[ORDER_I*N_CAC+cac]))*dt + sqrt((_r[cac][1]*X[ORDER_I*N_CAC+cac]))*dem_sto__1[cac];
        }
...

{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0; ac<N_AC; ac++) {
            ...
            f[0*N_CAC+cac] = X[0*N_CAC+cac] +  ( - (_r[cac][0]*X[ORDER_S*N_CAC+cac]))*dt + - sqrt((_r[cac][0]*X[ORDER_S*N_CAC+cac]))*dem_sto__0[cac] + - (_r[cac][0]*X[ORDER_S*N_CAC+cac])*par[ORDER_sto][routers[ORDER_sto]->map[cac]]*white_noise__0[cac];
            f[1*N_CAC+cac] = X[1*N_CAC+cac] +  ( - (_r[cac][1]*X[ORDER_I*N_CAC+cac]) + (_r[cac][0]*X[ORDER_S*N_CAC+cac]))*dt + - sqrt((_r[cac][1]*X[ORDER_I*N_CAC+cac]))*dem_sto__1[cac]+ sqrt((_r[cac][0]*X[ORDER_S*N_CAC+cac]))*dem_sto__0[cac] + + (_r[cac][0]*X[ORDER_S*N_CAC+cac])*par[ORDER_sto][routers[ORDER_sto]->map[cac]]*white_noise__0[cac];
        }
    }
...

{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0; ac<N_AC; ac++) {
            ...
            f[0*N_CAC+cac] = X[0*N_CAC+cac] +  ( - (_r[cac][0]*X[ORDER_S*N_CAC+cac]))*dt + - (_r[cac][0]*X[ORDER_S*N_CAC+cac])*par[ORDER_sto][routers[ORDER_sto]->map[cac]]*white_noise__0[cac];
            f[1*N_CAC+cac] = X[1*N_CAC+cac] +  ( - (_r[cac][1]*X[ORDER_I*N_CAC+cac]) + (_r[cac][0]*X[ORDER_S*N_CAC+cac]))*dt + + (_r[cac][0]*X[ORDER_S*N_CAC+cac])*par[ORDER_sto][routers[ORDER_sto]->map[cac]]*white_noise__0[cac];
        }
    }
    ...
    for (ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for (n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc += ( + (_r[cac][1]*X[ORDER_I*N_CAC+cac]))*dt ;
        }
...

{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0; ac<N_AC; ac++) {
            ...
            f[0*N_CAC+cac] = X[0*N_CAC+cac] +  ( - (_r[cac][0]*X[ORDER_S*N_CAC+cac]))*dt;
            f[1*N_CAC+cac] = X[1*N_CAC+cac] +  ( - (_r[cac][1]*X[ORDER_I*N_CAC+cac]) + (_r[cac][0]*X[ORDER_S*N_CAC+cac]))*dt;
        }
    }
